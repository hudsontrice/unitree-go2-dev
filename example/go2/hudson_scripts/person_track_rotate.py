#!/usr/bin/env python3
"""Ultra-basic person tracking (faces the largest detected persongi
Steps:
...
MODEL = "yolov8n.pt"       # small YOLOv8 model (fast). You can swap out for larger models
CONF_THRESHOLD = 0.25      # keep detections above this confidence                              [Needs tuning]
DETECT_INTERVAL = 2        # run the neural net every X frames, 1 = every frame
KP = 0.9                   # proportional gain (higher => turns more aggressively)              [Needs tuning]
MAX_YAW = 0.8              # maximum turn speed in radians/sec                                  [Needs tuning]  
DEADBAND_PX = 40           # if error smaller than this, do nothing (prevents twitching)        [Needs tuning]
...
def main():
    ...
    # Load YOLO model once returns in the form, .predict()
    ...
    print('StandUp...')
    sport.StandUp(); time.sleep(2)   # wait a bit for posture to stabilize before sending any move commands
    try:
        sport.BalanceStand(); time.sleep(0.3)  # switch into balance mode, also required for movement
    except Exception:
        pass  # not critical

    # Create the video client (front camera). Then grab one test frame to confirm that its working.
    ...
    try:
        while True:
            ...
            arr = np.frombuffer(bytes(data), dtype=np.uint8) # raw bytes -> numpy array
            frame = cv2.imdecode(arr, cv2.IMREAD_COLOR)  # decode JPEG -> BGR image
            if frame is None:
                continue
            frame_disp = frame if SCALE == 1.0 else cv2.resize(frame,(0,0),fx=SCALE,fy=SCALE) # So dislay doesn't have to be the entire frame

            # Run detection only every DETECT_INTERVAL frames
            best = None
            if (frame_idx - last_det_frame) >= DETECT_INTERVAL:  # run YOLO again
                rgb = cv2.cvtColor(frame_disp, cv2.COLOR_BGR2RGB)  # model wants RGB, swap it
            ...
            if best:
                ...
                cv2.rectangle(frame_disp,(int(x1),int(y1)),(int(x2),int(y2)),(0,0,255),2) # Red around person detected
                cv2.circle(frame_disp,(int(cx),int(0.5*(y1+y2))),4,(0,255,255),-1) # 
                cv2.line(frame_disp,(int(center_x),0),(int(center_x),frame_disp.shape[0]),(255,255,0),1) # Line for ref
